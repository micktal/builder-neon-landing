...
import { GraduationCap, Building2, FileText, ExternalLink, Mail, Download, Home, Search as SearchIcon, LayoutTemplate } from "lucide-react";
...
const DEMO_FORMATIONS = [
  {
    ...
    objectives: "Identifier les signaux de tension et adopter des techniques de désescalade.",
  ...
  },
  {
    ...
    objectives: "Sensibiliser aux risques HSE et mettre en place des plans d'action concrets.",
  ...
  },
  {
    ...
    objectives: "Maîtriser les gestes qui sauvent et la conduite à tenir en cas d'accident.",
...
const DEMO_PROSPECTS = [
  {
    ...
    size_band: "500-1000",
    region: "IDF",
    contacts: [{ name: "Claire Dupont", role: "DRH", email: "claire.dupont@example.com", phone: "+33 1 23 45 67 89" }],
  ...
  },
  {
    ...
    size_band: "1000+",
    region: "ARA",
    contacts: [{ name: "Marc Leroy", role: "Sécurité", email: "marc.leroy@example.com", phone: "+33 4 11 22 33 44" }],
  ...
  },
  {
    ...
    size_band: "200-500",
    region: "IDF",
    contacts: [{ name: "Sarah Martin", role: "Retail Manager", email: "s.martin@example.com", phone: "+33 6 22 33 44 55" }],
    training_history: "—",
    priority_score: 84,
...
const DEMO_TEMPLATES = [
  {
    ...
    email_body: "Bonjour {{contact_name}},\nNous aidons des enseignes retail sur {{formation_title}} ({{format}}).\nSeriez-vous disponible pour un échange ?\nCordialement, {{your_name}}",
    speech_text: "Bonjour {{contact_name}}, je suis {{your_name}} (FPSG). Sur des contextes retail, nous intervenons sur {{formation_title}}…",
  },
  {
...
export default function Resultats() {
  ...
  const emptyAll = formations.length === 0 && prospects.length === 0 && templates.length === 0;
    ...
    <div className="container max-w-[1200px] px-4 sm:px-6 pb-28 sm:pb-12">
      ...
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-gray-200">
        <div className="container max-w-[1200px] px-4 sm:px-6 py-3 flex items-center justify-between">
          <div>
            <h1 className="text-[20px] sm:text-[24px] font-bold text-slate-900">Résultats de recherche</h1>
            <div className="text-xs sm:text-sm text-slate-600">{summaryMini || "Filtres"}</div>
          </div>
      ...
      <section className="mt-6">
        <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900 mb-3">Votre recherche</h2>
        ...
        <section className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {formations.map((f, idx) => (
              <div key={idx} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow transition">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-slate-900">{f.title}</h3>
                  <span className="rounded-full bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 text-[11px]">{f.domain}</span>
                </div>
                <div className="mt-1 text-xs text-slate-600">{f.duration} • {f.format}</div>
                <div className="mt-2 text-sm text-slate-700">Publics: {f.audiences.join(", ")}</div>
                <div className="mt-1 text-sm text-slate-600 line-clamp-2">{f.objectives}</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <a href={f.pdf_brochure} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50">
                    <ExternalLink className="h-4 w-4" /> Voir plaquette PDF
                  </a>
                  <button onClick={() => copy("Lien teaser", f.teaser_url)} className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50">
      ...
      {prospects.length > 0 && (
        <section className="mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {prospects.map((p) => (
              <div key={p.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow transition">
                <div className="text-sm font-semibold text-slate-900">{p.company_name}</div>
                <div className="text-xs text-slate-600">{p.sector} • {p.size_band} • {p.region}</div>
                <div className="mt-2 text-sm">
                  {p.contacts.length > 0 && (
                    <div className="text-slate-700">
                      {p.contacts[0].name} — {p.contacts[0].role} • {p.contacts[0].email}
                    </div>
                  )}
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {p.contacts[0]?.email && (
                    <button onClick={() => copy("E-mail", p.contacts[0].email)} className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50">
                      <Mail className="h-4 w-4" /> Copier e-mail contact
                    </button>
                  )}
                  <Link to={`/prospects?id=${p.id}`} className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50">Ouvrir fiche</Link>
                </div>
              </div>
            ))}
          </div>
            ...
            {templates.map((t, idx) => (
              <div key={idx} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:shadow transition">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded-full bg-gray-100 text-slate-700 border border-gray-200 px-2 py-0.5 text-[11px]">{t.use_case}</span>
                </div>
                <div className="mt-3 space-y-2">
                  <div className="rounded-md border border-gray-200 bg-gray-50 p-2 text-sm text-slate-800 whitespace-pre-wrap">{t.speech_text}</div>
                  <div className="rounded-md border border-gray-200 bg-white p-2 text-sm text-slate-800">
                    <div className="text-xs text-slate-600">Subject: {t.email_subject}</div>
                    <div className="mt-1 whitespace-pre-wrap">{t.email_body}</div>
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-30 border-t border-gray-200 bg-white/95 backdrop-blur">
        <div className="mx-auto max-w-[1200px] grid grid-cols-3">
          <Link to="/dashboard" className="flex items-center justify-center gap-2 py-2 text-sm hover:bg-gray-50"><Home className="h-4 w-4" /> Dashboard</Link>
          <Link to="/nouvelle-recherche" className="flex items-center justify-center gap-2 py-2 text-sm hover:bg-gray-50"><SearchIcon className="h-4 w-4" /> Recherche</Link>
          <Link to="/templates" className="flex items-center justify-center gap-2 py-2 text-sm hover:bg-gray-50"><LayoutTemplate className="h-4 w-4" /> Templates</Link>
        </div>
      </nav>
    </div>
  );

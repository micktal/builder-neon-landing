// Minimal typings
declare global { interface Window { pdfMake?: any } }
...
export async function generateProposalPDF({ formation, prospect, contact, template, sales, costOverride }: any) {
  ...
  const price = (formation?.price_estimate && String(formation.price_estimate).trim()) || (costOverride || "Sur devis");
  const company = prospect?.company_name || "";
  const accroche = template?.email_body ? decodeURIComponent(template.email_body).replace(/%0D%0A/g, "\n") : "";
  ...
  coverStack.push(
    ...
    { canvas: [{ type:'line', x1:0, y1:10, x2:515, y2:10, lineWidth:1, lineColor:'#e5e7eb' }], margin:[0,8,0,16] },
  );

  const descr: any[] = [];
  if (accroche) {
    descr.push({ text: "Contexte & approche", style:"h3", margin:[0,0,0,6] });
    descr.push({ text: accroche.replace(/\n{3,}/g,"\n\n"), style:"p", margin:[0,0,0,12] });
  }
  descr.push(
    { text: "Description du module", style:"h3", margin:[0,0,0,6] },
    { text: objectives || "Objectifs détaillés à préciser lors du cadrage.", style:"p" }
  );

  const infoGrid = [
    [{ text:"Domaine", style:"label"}, { text: domain || "—", style:"cell" }],
    [{ text:"Durée", style:"label"}, { text: duration || "—", style:"cell" }],
    [{ text:"Format(s)", style:"label"}, { text: formats || "—", style:"cell" }],
    [{ text:"Public(s)", style:"label"}, { text: audiences || "—", style:"cell" }],
    [{ text:"Secteur(s) cible(s)", style:"label"}, { text: sectors || "—", style:"cell" }],
    [{ text:"Coût indicatif", style:"label"}, { text: price, style:"cell" }],
  ];
  ...
  const contactBlock = [
    ...
    { columns:[
      ...
      [
        ...
        { text: `${prospect?.sector || "—"} • ${prospect?.region || "—"}`, style:"p" },
        contact?.email ? { text: contact.email, style:"p" } : { text:"", margin:[0,0,0,0] },
      ]
    ], columnGap: 24 }
  ];
  ...
  const docDefinition = {
    ...
    footer: (currentPage: number, pageCount: number) => ({
      columns: [
        { text: 'Fiducial FPSG — Formation Prévention Sûreté Gestion', alignment:'left', style:'footer' },
    ...
    content: [
      ...
      { table: { widths:[160,'*'], body: infoGrid }, layout: {
    ...
    styles: {
      ...
      label:{ fontSize: 10, bold:true, color:'#374151', margin:[0,2,8,2]},

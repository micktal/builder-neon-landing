...
export default function ProspectsMap({
  ...
  const renderMarkers = () => {
    ...
    points.forEach((p) => {
      const color =
        mode === "score"
          ? colorByScore(p.priority_score)
          : sectorColor(p.sector);
      ...
      if (z >= 8)
        marker.bindTooltip(p.company_name, {
          permanent: false,
          direction: "top",
        });
    ...
    <div className="relative rounded-2xl border border-gray-200 bg-white p-2 shadow-sm">
      <div className="flex items-center justify-between px-2 pb-2">
        <div className="text-sm text-slate-700">
          Carte des prospects — couleur par{" "}
          {mode === "score" ? "score" : "secteur"}
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setMode("score")}
            className={`rounded-md border px-2 py-1 text-xs ${mode === "score" ? "bg-primary text-primary-foreground" : "bg-white"}`}
          >
            Score
          </button>
          <button
            onClick={() => setMode("sector")}
            className={`rounded-md border px-2 py-1 text-xs ${mode === "sector" ? "bg-primary text-primary-foreground" : "bg-white"}`}
          >
            Secteur
          </button>
        </div>
      ...
      <div className="px-2 pt-2 text-xs text-slate-500">
        Zoom pour révéler les labels. Cliquez sur un point pour ouvrir la fiche.
      </div>
    </div>
  );
}

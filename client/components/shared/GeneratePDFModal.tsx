...
export default function GeneratePDFModal({
  ...
  const [selectedProspect, setSelectedProspect] = useState<Prospect | null>(
    context === "prospect" ? initialProspect || null : null,
  );
  const [selectedFormation, setSelectedFormation] = useState<Formation | null>(
    context === "formation" ? initialFormation || null : null,
  );
  ...
  const [yourPhone, setYourPhone] = useState<string>(
    () => localStorage.getItem("fpsg_user_phone") || "",
  );

  useEffect(() => {
    if (!initialTemplates) {
      (async () => {
        const t = await fetch("/api/templates?limit=200")
          .then((r) => r.json())
          .catch(() => ({ items: [] }));
        setTemplates(
          Array.isArray(t?.items) ? t.items.map((x: any) => x.data) : [],
        );
      })();
    }
    if (!initialProspects) {
      (async () => {
        const { items } = await fetchBuilderContent<Prospect>("prospects", {
          limit: 200,
          cacheBust: true,
        });
        setProspects(items);
    ...
    if (context === "prospect") {
      (async () => {
        const f = await fetch("/api/formations?limit=200")
          .then((r) => r.json())
          .catch(() => ({ items: [] }));
        setFormations(
          Array.isArray(f?.items) ? f.items.map((x: any) => x.data) : [],
        );
      })();
    }
  ...
  useEffect(() => {
    if (
      context === "formation" &&
      initialFormation &&
      !initialFormation.price_estimate
    )
      setCostInput("");
  ...
  const filteredFormations = useMemo(() => {
    ...
    return formations.filter(
      (f: any) =>
        !s || `${f.title} ${f.domain || ""}`.toLowerCase().includes(s),
    );
  ...
  useEffect(() => {
    if (selectedProspect)
      setSelectedContact((selectedProspect.contacts || [])[0] || null);
  }, [selectedProspect?.company_name]);

  const domain = selectedFormation?.domain || initialFormation?.domain || "";
  const format0 = Array.isArray(
    selectedFormation?.format || initialFormation?.format,
  )
    ? (selectedFormation?.format || initialFormation?.format)![0]
    : ((selectedFormation?.format || initialFormation?.format || "") as any);
  ...
  const filteredTemplates = useMemo(() => {
    return (templates || []).filter(
      (t) =>
        ...
        (!sector ||
          !t.sector_filter ||
          (t.sector_filter || []).includes(String(sector))),
    );
  }, [templates, domain, format0, sector]);

  const canGenerate = () => {
    if (context === "formation")
      return !!(selectedProspect && (initialFormation || selectedFormation));
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const contact =
        selectedContact || (prospect.contacts && prospect.contacts[0]) || null;
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Combine la formation, le prospect et un template d'accroche pour
            créer une proposition PDF mise en page.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          {context === "formation" ? (
            <div>
              ...
              <Input
                value={qProspect}
                onChange={(e) => setQProspect(e.target.value)}
                placeholder="Rechercher prospect…"
                className="mt-1"
              />
              <select
                className="mt-2 w-full rounded-md border px-3 py-2 text-sm"
                value={selectedProspect ? selectedProspect.company_name : ""}
                onChange={(e) =>
                  setSelectedProspect(
                    filteredProspects.find(
                      (p) => p.company_name === e.target.value,
                    ) || null,
                  )
                }
              >
                ...
                {filteredProspects.map((p, i) => (
                  <option key={i} value={p.company_name}>
                    {p.company_name} — {p.sector || "—"} / {p.region || "—"}
                  </option>
                ))}
              </select>
          ...
          {context === "prospect" ? (
            <div>
              ...
              <Input
                value={qFormation}
                onChange={(e) => setQFormation(e.target.value)}
                placeholder="Rechercher formation…"
                className="mt-1"
              />
              <select
                className="mt-2 w-full rounded-md border px-3 py-2 text-sm"
                value={selectedFormation ? selectedFormation.title : ""}
                onChange={(e) =>
                  setSelectedFormation(
                    filteredFormations.find(
                      (f: any) => f.title === e.target.value,
                    ) || null,
                  )
                }
              >
                <option value="">— Choisir —</option>
                {filteredFormations.map((f: any, i: number) => (
                  <option key={i} value={f.title}>
                    {f.title} {f.domain ? `(${f.domain})` : ""}
                  </option>
                ))}
              </select>
            </div>
          ...
          {(!initialFormation?.price_estimate ||
            !String(initialFormation?.price_estimate).trim()) && (
            <div>
              <label className="text-sm font-medium">Coût indicatif</label>
              <Input
                value={costInput}
                onChange={(e) => setCostInput(e.target.value)}
                placeholder="ex. à partir de 2 900 € HT"
              />
            </div>
          )}

...
export const importProspects: RequestHandler = async (req, res) => {
  try {
    ...
    if (typeof (req as any).body === 'string' && /text\/(plain|csv)|octet-stream/.test(ctype)) {
      csv = (req as any).body as string;
    } else if (typeof (req as any).body?.csv === 'string' && (req as any).body.csv.trim()) {
    ...
    const required = ['company_name','sector','region','size_band','preferred_format','priority_score','contacts','stage','notes','createdAt'];
    const missing = required.filter(h => !header.includes(h));
    if (missing.length) return res.status(400).json({ error: `En-têtes manquants: ${missing.join(', ')}` });
    ...
    const items = rows.slice(1).map(cols => {
      ...
      const createdAt = cols[iCreated] && !isNaN(Date.parse(cols[iCreated])) ? new Date(cols[iCreated]).toISOString() : new Date().toISOString();
      const data: any = {
        company_name,
        sector: cols[iSector] || '',
        region: cols[iRegion] || '',
    ...
    for (const data of items) {
      ...
      if (r?.action === 'created') created++; else if (r?.action === 'updated') updated++;
      results.push({ id: r?.id || r?._id || null, company_name: data.company_name, action: r?.action });
    }

    return res.json({ ok: true, created, updated, errors_count: 0, items: results });
  } catch (e: any) {
